...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        /// <summary>
        /// Add a reset ammo method, so we can easily reset a weapon ammo manually, without reloading scene
        /// </summary>
        public void ResetAmmo()
        {
            m_CurrentAmmo = MaxAmmo;
            CurrentAmmoRatio = 1.0f;
            IsCooling = false;
        }
        ...
        void UpdateContinuousShootSound()
        {
            if (UseContinuousShootSound)
            {
                ...
                {
                    if (!m_ContinuousShootAudioSource.isPlaying && !trainingMode)
                    {
                ...
                else if (m_ContinuousShootAudioSource.isPlaying && !trainingMode)
                {
        ...
        public void ShowWeapon(bool show)
        {
            ...
            if (show && ChangeWeaponSfx && !trainingMode)
            {
        ...
        void HandleShoot()
        {
            ...
            // play shoot SFX
            if (ShootSfx && !UseContinuousShootSound && !trainingMode)
            {
                m_ShootAudioSource.PlayOneShot(ShootSfx);
            }
