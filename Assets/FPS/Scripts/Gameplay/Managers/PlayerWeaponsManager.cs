    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (agentControl)
            {
                ...
                Debug.AssertFormat(agent, nameof(PlayerWeaponsManager) + "<color=yellow>Warning</yellow> could not find attached agent!");
            }
        ...
        void Update()
        {
            ...
            if (activeWeapon != null && m_WeaponSwitchState == WeaponSwitchState.Up)
            {
                ...
                if (agentControl) reloadButtonDown = agent.GetReloadButtonDown();
                else reloadButtonDown = m_InputHandler.GetReloadButtonDown();

                if (!activeWeapon.AutomaticReload && reloadButtonDown && activeWeapon.CurrentAmmoRatio < 1.0f)
                {
                    IsAiming = false;
                ...
                if (agentControl) IsAiming = agent.GetAimInputHeld();
                ...
                if (agentControl)
                {
                    hasFired = activeWeapon.HandleShootInputs(
                        agent.GetFireInputDown(),
                        agent.GetFireInputHeld(),
                        agent.GetFireInputReleased());
                }
                else
                {
                    hasFired = activeWeapon.HandleShootInputs(
